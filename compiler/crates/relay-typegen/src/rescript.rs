...
// This is the same as TopLevelFragmentType, but for the response type.
// Mutations can be Results, otherwise everything as a response is always an object.
#[derive(Debug)]
pub enum TopLevelResponseType {
    Object(Object),
    Result(Object),
}
...
pub struct ReScriptPrinter {
    ...
    // response top level.
    pub response: Option<(bool, TopLevelResponseType)>,
...
fn write_enum_util_functions(state: &ReScriptPrinter, str: &mut String, indentation: usize, full_enum: &FullEnum) -> Result {
    ...
    if !state.no_future_proof_enums {
    ...
    if state.no_future_proof_enums {
        write_indentation(str, indentation + 1).unwrap();
        writeln!(str, "switch str {{").unwrap();
        for enum_value in &full_enum.values {
            write_indentation(str, indentation + 2).unwrap();
            writeln!(
                str,
                "| \"{}\" => Some({})",
                enum_value,
                capitalize_string(enum_value),
            )
            .unwrap();
        }
        write_indentation(str, indentation + 2).unwrap();
...
fn write_fragment_definition(
    ...
    match &fragment {
        ...
        &TopLevelFragmentType::Result(obj) => {
            if nullable {
                write_object_definition(
                    state,
                    str,
        ...
        &TopLevelFragmentType::ArrayWithResult(obj) => {
            write_object_definition(
                state,
                str,
                indentation,
        ...
        match &preloadable_mode {
            &PreloadableMode::Off | &PreloadableMode::FullFile => {
                ...
                if let Some((nullable, response)) = &self.response {
                    ...
                    match response {
                        TopLevelResponseType::Object(response) => {
                            if *nullable {
                                write_object_definition(
                                    &self,
                                    &mut generated_types,
                        ...
                        TopLevelResponseType::Result(response) => {
                            write_object_definition(
                                &self,
                                &mut generated_types,
                                indentation,
            ...
            match cto {
                ...
                Some((
                    ...
                    self.fragment = Some((
                        nullable,
                        if is_result {
                            TopLevelFragmentType::ArrayWithResult(fragment_type)
                        } else {
...
mod tests {
    ...
    fn make_printer(no_future_proof_enums: bool, no_future_proof_unions: bool) -> ReScriptPrinter {
        ...
        let ast = parse_executable(
            ...
            SourceLocationKey::standalone("TestFragment.graphql"),
        )
        .unwrap();
    ...
    fn enum_utils_use_explicit_string_matching_when_future_proofing_is_disabled() {
        ...
        write_enum_util_functions(&state, &mut output, 0, &full_enum).unwrap();

        assert!(!output.contains("FutureAddedValue"));
        assert!(output.contains("| \"Online\" => Some(Online)"));
